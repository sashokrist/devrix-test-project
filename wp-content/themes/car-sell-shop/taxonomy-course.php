<?php
/**
 * The template for displaying course taxonomy archives
 *
    ...
    <main id="main" class="site-main">
        <div class="page-header">
            <h1 class="page-title">
                ...
                if ( $current_term ) {
                    printf( esc_html__( 'Course: %s', 'car-sell-shop' ), $current_term->name );
                } else {
                    esc_html_e( 'Courses', 'car-sell-shop' );
                }
                ?>
            </h1>
        ...
        <?php else : ?>
            <div class="no-students">
                <p><?php esc_html_e( 'No students found in this course.', 'car-sell-shop' ); ?></p>
            </div>
        <?php endif; ?>

        <?php
        // Display other courses
        $other_courses = get_terms( array(
            'taxonomy' => 'course',
            'hide_empty' => true,
            'exclude' => $current_term ? array( $current_term->term_id ) : array(),
        ) );

        if ( $other_courses && ! is_wp_error( $other_courses ) ) : ?>
            <div class="other-courses">
                <h3><?php esc_html_e( 'Other Courses', 'car-sell-shop' ); ?></h3>
                <div class="courses-list">
                    <?php foreach ( $other_courses as $course ) : ?>
                        <a href="<?php echo esc_url( get_term_link( $course ) ); ?>" class="course-link">
                            <?php echo esc_html( $course->name ); ?>
                            <span class="course-count">(<?php echo esc_html( $course->count ); ?>)</span>
                        </a>
                    <?php endforeach; ?>
                </div>
            </div>
