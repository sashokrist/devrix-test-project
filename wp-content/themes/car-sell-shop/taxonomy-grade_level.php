<?php
/**
 * The template for displaying grade_level taxonomy archives
 *
 * @package Car Sell Shop
 * @since 1.0.0
 */

get_header();
?>

<div id="primary" class="content-area">
    <main id="main" class="site-main">
        <div class="page-header">
            <h1 class="page-title">
                <?php
                $current_term = get_queried_object();
                if ( $current_term ) {
                    printf( esc_html__( 'Grade Level: %s', 'car-sell-shop' ), $current_term->name );
                } else {
                    esc_html_e( 'Grade Levels', 'car-sell-shop' );
                }
                ?>
            </h1>
            <?php if ( $current_term && $current_term->description ) : ?>
                <div class="archive-description">
                    <?php echo wp_kses_post( $current_term->description ); ?>
                </div>
            <?php endif; ?>
        </div>

        <?php if ( have_posts() ) : ?>
            <div class="students-grid">
                <?php while ( have_posts() ) : the_post(); ?>
                    <article id="post-<?php the_ID(); ?>" <?php post_class( 'student-card' ); ?>>
                        <div class="student-featured-image">
                            <?php if ( has_post_thumbnail() ) : ?>
                                <a href="<?php the_permalink(); ?>">
                                    <?php the_post_thumbnail( 'medium' ); ?>
                                </a>
                            <?php endif; ?>
                        </div>

                        <div class="student-content">
                            <h2 class="student-title">
                                <a href="<?php the_permalink(); ?>"><?php the_title(); ?></a>
                            </h2>

                            <div class="student-meta">
                                <?php
                                // Get student meta data
                                $student_id = get_post_meta( get_the_ID(), '_student_id', true );
                                $student_email = get_post_meta( get_the_ID(), '_student_email', true );
                                $student_phone = get_post_meta( get_the_ID(), '_student_phone', true );
                                $student_dob = get_post_meta( get_the_ID(), '_student_dob', true );
                                $student_address = get_post_meta( get_the_ID(), '_student_address', true );
                                ?>

                                <?php if ( $student_id ) : ?>
                                    <div class="meta-item">
                                        <strong><?php esc_html_e( 'ID:', 'car-sell-shop' ); ?></strong> <?php echo esc_html( $student_id ); ?>
                                    </div>
                                <?php endif; ?>

                                <?php if ( $student_email ) : ?>
                                    <div class="meta-item">
                                        <strong><?php esc_html_e( 'Email:', 'car-sell-shop' ); ?></strong> <?php echo esc_html( $student_email ); ?>
                                    </div>
                                <?php endif; ?>

                                <?php if ( $student_phone ) : ?>
                                    <div class="meta-item">
                                        <strong><?php esc_html_e( 'Phone:', 'car-sell-shop' ); ?></strong> <?php echo esc_html( $student_phone ); ?>
                                    </div>
                                <?php endif; ?>

                                <?php if ( $student_dob ) : ?>
                                    <div class="meta-item">
                                        <strong><?php esc_html_e( 'Date of Birth:', 'car-sell-shop' ); ?></strong> <?php echo esc_html( $student_dob ); ?>
                                    </div>
                                <?php endif; ?>
                            </div>

                            <div class="student-taxonomies">
                                <?php
                                // Display course terms
                                $courses = get_the_terms( get_the_ID(), 'course' );
                                if ( $courses && ! is_wp_error( $courses ) ) : ?>
                                    <div class="taxonomy-terms">
                                        <strong><?php esc_html_e( 'Courses:', 'car-sell-shop' ); ?></strong>
                                        <?php
                                        $course_names = array();
                                        foreach ( $courses as $course ) {
                                            $course_names[] = '<a href="' . esc_url( get_term_link( $course ) ) . '">' . esc_html( $course->name ) . '</a>';
                                        }
                                        echo implode( ', ', $course_names );
                                        ?>
                                    </div>
                                <?php endif; ?>

                                <?php
                                // Display grade level terms
                                $grade_levels = get_the_terms( get_the_ID(), 'grade_level' );
                                if ( $grade_levels && ! is_wp_error( $grade_levels ) ) : ?>
                                    <div class="taxonomy-terms">
                                        <strong><?php esc_html_e( 'Grade Levels:', 'car-sell-shop' ); ?></strong>
                                        <?php
                                        $grade_names = array();
                                        foreach ( $grade_levels as $grade ) {
                                            $grade_names[] = '<a href="' . esc_url( get_term_link( $grade ) ) . '">' . esc_html( $grade->name ) . '</a>';
                                        }
                                        echo implode( ', ', $grade_names );
                                        ?>
                                    </div>
                                <?php endif; ?>
                            </div>

                            <div class="student-excerpt">
                                <?php the_excerpt(); ?>
                            </div>

                            <a href="<?php the_permalink(); ?>" class="read-more-btn">
                                <?php esc_html_e( 'View Student', 'car-sell-shop' ); ?>
                            </a>
                        </div>
                    </article>
                <?php endwhile; ?>
            </div>

            <?php
            // Pagination
            the_posts_pagination( array(
                'mid_size'  => 2,
                'prev_text' => __( '&laquo; Previous', 'car-sell-shop' ),
                'next_text' => __( 'Next &raquo;', 'car-sell-shop' ),
            ) );
            ?>

        <?php else : ?>
            <div class="no-students">
                <p><?php esc_html_e( 'No students found in this grade level.', 'car-sell-shop' ); ?></p>
            </div>
        <?php endif; ?>

        <?php
        // Display other grade levels
        $other_grade_levels = get_terms( array(
            'taxonomy' => 'grade_level',
            'hide_empty' => true,
            'exclude' => $current_term ? array( $current_term->term_id ) : array(),
        ) );

        if ( $other_grade_levels && ! is_wp_error( $other_grade_levels ) ) : ?>
            <div class="other-grade-levels">
                <h3><?php esc_html_e( 'Other Grade Levels', 'car-sell-shop' ); ?></h3>
                <div class="grade-levels-list">
                    <?php foreach ( $other_grade_levels as $grade_level ) : ?>
                        <a href="<?php echo esc_url( get_term_link( $grade_level ) ); ?>" class="grade-level-link">
                            <?php echo esc_html( $grade_level->name ); ?>
                            <span class="grade-level-count">(<?php echo esc_html( $grade_level->count ); ?>)</span>
                        </a>
                    <?php endforeach; ?>
                </div>
            </div>
        <?php endif; ?>
    </main>
</div>

<?php
get_footer();
?>
